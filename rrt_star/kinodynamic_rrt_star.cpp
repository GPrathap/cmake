#include "kinodynamic_rrt_star.h"
#include <sstream>
...
int KinodynamicRRTstar::search(Eigen::Vector3d start_pt, Eigen::Vector3d start_v, Eigen::Vector3d start_a,
                             Eigen::Vector3d end_pt, Eigen::Vector3d end_v, int number_of_obs, bool init, std::vector<Eigen::Vector3d> curr_range
                             , bool dynamic, double time_start, double increase_cleareance, int path_index)
{
  ...
  for (int i = 0; i < times; i++) {
      ...
      X.init_search_space(x_dimentions, 4, rrt_avoidance_dist_mod, 10);
      ...
      rrtstart3d->rrt_init(rewrite_count, rrt_planner_options, common_utils, i*3);
      push_job(rrtstart3d);
  }
